...
- **Stochastic-order (base world):** the departing request is $J \sim \mathrm{Uniform}\{1,\dots,n\}$ among in-flight. Q3 (in-flight set and order) is substantive, and the provenance rung has its D2 grip — lineage on a completion record resolves *which* request finished, moving the Q3/Q4 posteriors.
- **FIFO (control world):** the departing request is the head; completion order equals issue order, only timing stochastic. Lineage becomes *derivable* — the k-th completion is the k-th issue — making it exactly D2's redundant-telemetry control, in the same schema, same field, same vocabulary. Cleaner than a synthetic derivable field.
...
**M must be declared per analysis, with sign convention** (v0.2.2 — score-type M: higher better; loss/entropy-type M: signs reverse). At the **Bayes ceiling under expected log score with full context**, the gain of lineage is exactly the conditional mutual information $I(Z; L \mid H)$ — zero in FIFO (derivable), positive in stochastic-order, positive again in FIFO under truncation. **The name Δ_information is reserved for this exact-ceiling contrast**; for any learned-model metric, Δ_stoch − Δ_FIFO is an *interaction contrast*, not an information measurement.

For a **learned model**, Δ_FIFO need not be zero — and that is the more interesting reading: redundant lineage spares the model long-range matching, queue simulation, and counting. A nonzero learned Δ_FIFO therefore **operationalizes computational accessibility** — information present implicitly but requiring computation, versus handed over as telemetry — *under a matched training protocol, with irrelevant-field controls, multiple seeds, and scaling checks required to rule out simpler explanations* (finite-data effects, optimization variance, token-frequency and calibration differences, direct-copy shortcuts, positional biases). The conceptual three-state distinction stands — a fact can be unavailable, implicitly derivable, or explicitly exposed — but these are three degrees of **trace accessibility**, not the three pipeline gaps (mapping corrected v0.2.3): they sit entirely on the world→trace→computation side of $X_t \to h_t \to A_t \to Y_t$. Comparing exact ceilings, learned representations, and outputs across the three conditions helps isolate observation and accessible-representation losses; the exposure/use gap remains a separate activation-to-output comparison — a model can receive lineage directly, represent it internally, and still fail to expose or use it. The learned contrast evidences the middle state; it does not identify it causally without the surrounding controls.
...
- **C0b (device validation):** 2 threads, 1 CPU, 1 device, queue depth 2, minimal workload placing two requests in flight. Witnesses stochastic-order completion of a non-head request, lineage disambiguation, the FIFO/stochastic informational difference, the D10 crossover, and (under the D8 shuffled channel) a bucket containing two noncommuting events.
...
4. The full record schema — every field explicit, including EVENT_KIND and TIME_CLASS — and one observation model per interface rung: a deterministic projection $\pi_r$, or for order-coarsened rungs a stochastic observation kernel $\Pi_r(\cdot \mid E_{bk+1:bk+k})$ with its channel likelihood stated (D8).
...
12. *(v0.2.2)* The **interface-witness test suite**: machine-checkable witness histories for every interface claim — object, related-entity, and lineage each change a named posterior; the irrelevant control changes no ceiling; FIFO lineage is null at full context and positive under truncation; shuffled order changes a posterior on a noncommuting bucket. D2 becomes regression tests, not prose that can drift from the implementation.
