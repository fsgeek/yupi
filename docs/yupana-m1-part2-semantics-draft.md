...
- **Rung collapse (numerical criterion):** adjacent rungs r, r′ collapse at context $L$ if $\max_{q \in Q}\, [H_r(q \mid L) - H_{r'}(q \mid L)] < \delta$ bits, with $\delta$ frozen before any *confirmatory* curve is computed. *(v0.2.4: not yet frozen — the D4 freeze covered B1–B4 only; every adjacent-rung comparison of 2026-08-13…15 is exploratory relative to $\delta$. Freeze path: one sweep of adjacent-rung gaps over the COMPLETE statutory query set $\{$Q1, Q2, Q3, Q4 (statutory, split), Q5 per pair$\}$ — the day-seven raw JSONs already hold these for three laws — then the second stamped decision.)*
- **Synchronization horizon:** smallest $L$ at which a rung's posterior entropy on a query falls below $\delta_{sync}$ (frozen likewise). *(v0.2.4: not yet frozen; requires an $L$ sweep at fixed $T_{ep}$ — only $L \in \{2, 4\}$ exist at $T_{ep}=14$ today — not derivable from the rung-gap data; second stamped decision.)*
- **Divergence thresholds $(\delta_p, \Delta_\tau)$** *(v0.2.4)*: total-variation distances per §5's criterion form; frozen from the distributions of pairwise TV between windows' P-next mixtures and between their $\tau$ mixtures (sensitivity curves of pair_prob against $(\delta_p, \Delta_\tau)$); nothing measured through 2026-08-15 is this quantity except its $\delta_p = 0$ corner. Second stamped decision.
