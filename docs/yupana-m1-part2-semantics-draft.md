# Yupana Milestone 1 — Part II: Formal Semantics (Draft v0.2.5)

**Status:** exploratory draft, 2026-08-12; current through the 2026-08-13 correction round, Part I v0.2.4, the 2026-08-15 v0.2.4 amendment (predictive parameters, continuation semantic, threshold procedure), and the 2026-08-21 v0.2.5 enactment (the second stamped decision: thresholds frozen). This is the statute to Part I's constitution (`yupana-m1-spec-draft.md`). Every normative choice below carries rationale; contested choices carry a falsifier or revisit trigger. Attack freely.
...
**v0.2.1 (2026-08-13, cross-document correction round):** the fixed-horizon window measure is made consistent with terminal episodes; unanchored inference retains the latent window position jointly rather than averaging anchored posteriors with fixed weights; context length $L$ and D8 bucket size $B$ receive distinct notation. The shifted-reference rule now separates exact rational density ratios from numerical logarithms and reports observation shift separately from conditional-target shift. §9 and Part I D10 are reconciled with the full-context redundancy measured at `3e5a19f`. `CLAUDE.md` is re-graded as founding orientation rather than normative authority. These are corrections to v0.2's enacted direction, not a reversal of it. *(Provenance: round authored by ChatGPT at Tony's request, commit `a0e20ab`; reviewed by the Claude instance of the same day — review applied the v0.2.2 scoping amendment below.)*

**v0.2.2 (2026-08-13, review amendment):** witnesses 1–2 rescoped. As inherited, the §9 preamble scoped witnesses 1–3 to C0-family window enumeration, but for witnesses 1 and 2 that search is provably empty by arity at any context length: every C0 configuration has $n_L, n_D \le 1$, making OBJECT a deterministic function of EVENT_KIND (witness 1), and C0a's $n_T = 2$ makes the blocked lock's owner the non-actor by elimination (witness 2). These are the same genus of error as the depth-1/D10 collapse — a configuration extensionally unable to express the distinction it is asked to witness. Witnesses 1–2 now run over C1 windows (or a future C0d added for the purpose); witnesses 3 and 6 remain C0b-scoped, where the arity argument does not apply (request identity under truncation is genuinely ambiguous).

**v0.2.4 (2026-08-15, predictive-parameter freeze and threshold procedure — enacted from `docs/part2-amendment-proposal-v0.2.4.md`):** the day-seven truthsayer round found that §5's "$m$, and $\mathcal{T}$ itself, frozen with the budgets in §7" and §6's "$\delta$ frozen alongside budgets before any curve is computed" had never been discharged — the D4 freeze (`d4-budget-freeze-v0.1.md`, commit `e921c74`) covers B1–B4 only — so every rung-gap and predictive result of days five–seven is exploratory relative to those parameters and is so labeled in its note. This amendment (A) freezes $m = 2$, $W = 4$ primary / $W = 8$ secondary, and $\mathcal{T}$ as enumerated in §5; (B) makes explicit that forward sums for Q4 and the predictive targets continue the time-homogeneous kernel past $T_{ep}$; (C) freezes **no** numerical threshold — $\delta$, $\delta_{sync}$, $\delta_p$, $\Delta_\tau$ are different quantities in different units and get three separate sweeps followed by a *second* stamped decision; (D) fixes the *form* of the divergent-history criterion (total-variation, prevalence as pair_prob). Version numbering: proposal v0.2.3 was revised to v0.2.4 on the third truthsayer pass before enactment, so Part II goes v0.2.2 → v0.2.4 with no enacted v0.2.3. Enacted on Tony's confirmation, 2026-08-15, quoted verbatim: *"I confirm docs/part2-amendment-proposal-v0.2.4.md as written. Sections A, B, and D-form bind now. Section C freezes no numerical thresholds; it establishes the three-sweep, second-stamp procedure. Please enact it as Part II v0.2.4 and stamp it."* Only measurements after the enacting commit's stamp are confirmatory with respect to $m$, $W$, $\mathcal{T}$, and the continuation semantic; threshold-dependent numerical claims stay exploratory until the second stamp.

**v0.2.5 (2026-08-21, threshold freeze — the second stamped decision, enacted from `docs/part2-threshold-freeze-proposal-v0.3.1.md`):** the three §C sweeps ran (buggy-kernel, 2026-08-15), the direct-handoff kernel erratum forced a corrected-kernel rerun (`docs/sweep-rerun-comparison-2026-08-21.md`, commit `36e8434`: every decision-bearing quantity unchanged), and the PI reaffirmed the proposal. Enacted: **δ = 0.01 bits** (§6 rung collapse); **δ_sync = 0.01 bits** on the truncation-conditional mean (proposal §B1, semantics now in §6) with Q4 entering by its gap part (proposal §A′); **Δ_τ = 0.01**; **δ_p reported as the axis {0, 10⁻⁴, 10⁻³, 10⁻², 3·10⁻²}** with the exact δ_p = 0 anchor — an axis, not a scalar, the convention kept visible per the proposal's §G2. PI confirmation quoted verbatim in the proposal's appended decision record (2026-08-21), which also carries the §C evidence-wording amendment (print-precision flatness corrected) and the instance's stated interpretation of the informal reaffirmation as the §F confirmation. Threshold-dependent claims measured before the enacting commit's stamp remain exploratory; confirmatory measurement at these thresholds requires fresh held-out laws (next queue item).
...
**Episode boundary and length:** an explicit RESET record $O_0$, visible at **every** rung when it falls within the available context *(rationale: episode-boundary observability is not an interface rung under study in M1; making the field universal keeps the ladder about within-episode information)*. The window-generating process always produces transition records $O_1,\ldots,O_{T_{ep}}$ through a fixed horizon $T_{ep}$, chosen as a multiple of D8 bucket size $B$. If all threads terminate earlier, §3.6's absorbing terminal IDLE self-loop supplies the remaining records. Thus “episode” means the fixed record sequence for corpus and exact-measure purposes; a simulator UI may stop displaying at termination, but that presentation choice does not change the training path measure.

**Continuation past $T_{ep}$** *(v0.2.4, amendment §B):* for predictive targets and Q4, the forward sum runs the kernel from the evaluation time $t$ irrespective of $T_{ep}$. $T_{ep}$ is the generated-record horizon of the episode law, not a termination of the world; the kernel is time-homogeneous. Consequently a window ending at $T = T_{ep}$ has the same forecast semantics as any other endpoint. *(Rejected alternative: the world halts at $T_{ep}$, so forecasts at $T = T_{ep}$ have NONE mass 1 — this would make the last endpoint of every law a degenerate control and tie a query's ceiling to a sampling convention. Codex's independent recommendation matched the adopted reading. Effect size: $\ge 1/|\mathcal E_B|$ of law mass per law.)*
...
*(a) Training window protocol.* Generate $O_{0:T_{ep}}$ as above. Let $L$ be the context horizon in transition records, constrained to a multiple of $B$, and draw a delivered-bucket endpoint

$$
T \sim \mathrm{Uniform}(\mathcal E_B), \qquad \mathcal E_B=\{B,2B,\ldots,T_{ep}\}, \qquad U=\max(0,T-L).
$$

The serialized window is

$$
H^{(L)}_T =
\begin{cases}
(O_0,O_1,\ldots,O_T), & U=0,\\
(O_{U+1},\ldots,O_T), & U>0.
\end{cases}
$$

Thus every truncated window contains complete delivered buckets; RESET is included exactly when the window reaches episode start and is not charged against the $L$-transition-record horizon. Its presence and the resulting one-record length difference are intentional synchronization evidence. **The truncation prior is derived, not stipulated:** the exact-filter prior is the marginal induced by this joint episode/endpoint/window law. Rationale: a Bayes ceiling is a ceiling *for a learner* only under the learner's own generative process; any stipulated prior the training distribution does not induce converts prior mismatch into a phantom accessible-representation term and the decomposition stops decomposing.

*(b) Base condition: offset-unanchored.* The observer is not given $T$ or $U$. Exact inference therefore retains a **joint** belief over latent start offset and state $(U,S_U)$. Equivalently, an implementation may retain one anchored component per compatible $U$ together with that component's evidence likelihood. After observing $h$, the mixture weights are posterior weights $P(U\mid h)$; they are not frozen at their sampling-prior values. Window length, the presence of RESET, and record content are legitimate evidence about position and must update those weights. This inverts v0.1's default (offset known, "lost history, not lost clock"): review found the absolute offset is a covert information channel that partially undoes the truncation it rides on. The anchored condition remains defined and is priced under (e); v0.1's falsifier anticipated the offset-unknown variant — the round made it the base.
...
- A **model-generalization row** (shifted ceiling $f_Q$; transplanted reference $f_P$; model $f_M$) is admissible only under process-level absolute continuity. When $P$ and $Q$ share a common joint episode-path/endpoint space $J$, require $Q_J \ll P_J$; the relevant measurable marginals then inherit absolute continuity. At the scored interface, verify and report both $Q_H \ll P_H$ (the transplant is defined on $Q$'s observations) and $Q_{H,Z} \ll P_{H,Z}$ (expected log loss is finite). If the first holds but the second fails, the transplant is defined but receives infinite log-loss cost; report that result rather than dropping the row. *Process* support is the criterion, not empirical-sample support: $f_P$ is defined on unsampled-but-possible windows — the same fact D3's exchangeable split relies on.
- Under the declared risk $R_Q$, decomposition and names are $R_Q(f_M) - R_Q(f_Q) = \underbrace{[R_Q(f_P) - R_Q(f_Q)]}_{\text{transplant cost, } \ge 0} + \underbrace{[R_Q(f_M) - R_Q(f_P)]}_{\text{learner residual, signed}}$. The nonnegativity follows because $f_Q$ is Bayes-optimal for that risk. The transplant cost prices the *unchanged $P$-optimal rule* under $Q$; it is not a floor for all learners, and a model may outperform the transplant (negative residual).
- Mandatory per-row diagnostics separate admissibility, reweighting severity, conditional-target shift, and finite-sample coverage. Report: (i) the range of $Q_H(h)/P_H(h)$ over $h$ with $Q_H(h)>0$ as exact rational ratios; any logarithmic rendering at declared numerical precision; (ii) $D_{\mathrm{KL}}(Q_H \| P_H)$; (iii) the conditional-target divergence

  $$
  \mathbb{E}_{h\sim Q_H} D_{\mathrm{KL}}\!\left(Q(Z\mid h)\,\|\,P(Z\mid h)\right),
  $$

  which equals transplant cost under expected log loss; and (iv) the joint check

  $$
  D_{\mathrm{KL}}(Q_{H,Z}\|P_{H,Z}) = D_{\mathrm{KL}}(Q_H\|P_H) + \mathbb{E}_{h\sim Q_H}D_{\mathrm{KL}}\!\left(Q(Z\mid h)\,\|\,P(Z\mid h)\right).
  $$

  KL terms and log ratios are transcendental in general and are reported at declared precision; probability measures and density ratios remain exact Fractions. Also report the $Q$-mass of windows absent from the empirical training sample and a $Q$-weighted summary of empirical training counts. **Artifact requirement:** the training-sample window census is a retained deliverable; the coverage diagnostics are computable only if it exists.
...
Field semantics per kind: ACTOR = the transitioning thread (or MASKED for IDLE/RESET); OBJECT = lock/device acted on; RELATED = current owner (on BLOCK), woken thread (on RELEASE), dispatching... (full table below); LINEAGE = request id (IO events) or MASKED; TIME_CLASS = absolute bucket index $\lfloor(t-1)/B\rfloor$ for transition record $O_t$ under D8 bucket size $B$, **maskable** *(v0.2)*: the base truncation condition masks it (offset-unanchored, §2b–c) and the anchored condition unmasks it — same schema, same vocabulary, model claims only by coarsening or promotion per §2e. RESET's TIME_CLASS is 0 when exposed.
...
**Order modes (D8):** each content rung crosses with order mode ∈ {ordered, shuffled}, both bucketed at size $B$, delivered at bucket boundaries, queries scored at boundaries only. Ordered: identity serialization, deterministic projection $\pi_r$. Shuffled: uniform within-bucket permutation — a stochastic kernel $\Pi_r(O_b \mid E_{bB+1:bB+B})$ whose likelihood of a visible sequence is $m/B!$ where $m$ is the number of permutations of the latent bucket producing it (duplicate records make $m > 1$). The filter multiplies world-path probability by this channel likelihood; it never assumes uniformity over causally possible latent orders.
...
- **Q4:** the thread *directly* woken by the first wake-causing transition in $(t, t + W]$ — for a completion, the issuer of the departing request; for a release-with-waiter, the woken head. Secondary wakes (QUEUE_BLOCKED threads freed by the same completion, §3.3) are excluded, keeping the value single-valued. Value in $\{1..n_T\} \cup \{\mathrm{NONE\_WITHIN\_W}\}$, horizon $W$ a fixed parameter — **frozen v0.2.4: $W = 4$ primary, $W = 8$ secondary** (rationale in the amendment §A: at $W=4$, 93/112 (ε=1) and 165/200 (ε=½) horizon-8 endpoint states have a non-degenerate wake forecast; $W=8$ makes all 312 non-degenerate; these are exploratory selections promoted, not derived optima). The forward sum continues the kernel past $T_{ep}$ (§2, v0.2.4). **Computation** *(Part II item 9)*: finite-horizon forward sum over the belief-state–conditioned kernel — for each support state, a $W$-step absorbing computation where absorption = first wake event, tagged by woken thread; cost $O(W \cdot |\mathrm{supp}| \cdot b)$ ($b$ = branching), budgeted separately from the forward filter and reported in the M1 report. *(Q4 is the query whose entropy has an irreducible term under exact state knowledge — Part I D1 correction; both terms reported.)*
...
- **P-horizon:** a preregistered finite test set $\mathcal{T}$ of future-observation functionals — for M1: the distribution of the next $m$ EVENT_KINDs; the identity of the next IO_COMPLETE's LINEAGE (where exposed); and the time-to-next-wake distribution truncated at $W$ — each computed exactly by finite-horizon forward sum over the belief-conditioned kernel. **Frozen v0.2.4 (amendment §A):** $m = 2$ (the smallest $m$ whose functional is finer than P-next's kind marginal); $W = 4$ primary / $8$ secondary as for Q4; $\mathcal{T} = \{$next-2 EVENT_KINDs; time-to-next-wake $\le W$; LINEAGE of the next IO_COMPLETE $\le W\}$, no additions. *(v0.2.2 text read "frozen with the budgets in §7"; the D4 freeze did not include them — see the v0.2.4 header block.)*
- **Divergent-history search:** enumeration (C0 family) and guided search (C1) for history pairs with equal P-next but unequal P-horizon on some $\tau \in \mathcal{T}$ — the immediate-agree/later-diverge classes that the exposure experiments require. Their existence, prevalence, and the interfaces under which they arise are an M1 exit deliverable (Part I deliverable 5); predictive equivalence ("histories inducing the same distribution over all futures") is the quotient these tests probe from the finite side. **Criterion form (v0.2.4, amendment §D):** two histories $h, h'$ under one condition are **$(\delta_p, \Delta_\tau)$-divergent** if $\mathrm{TV}(\text{P-next}(h), \text{P-next}(h')) \le \delta_p$ and, for some $\tau \in \mathcal{T}$, $\mathrm{TV}(\tau(h), \tau(h')) \ge \Delta_\tau$. Exact equality is the $\delta_p = 0$ corner and the day-seven baseline (`c1-predictive-targets-v0.1.md`, v0.2 block). Prevalence is reported as pair_prob (probability that two law-weighted draws are $(\delta_p, \Delta_\tau)$-divergent) and its TV-weighted form; law-mass coverage is reported alongside, not as the headline. The numerical thresholds are frozen at §6 (v0.2.5: $\Delta_\tau = 0.01$, $\delta_p$ as a reporting axis).
...
- **Rung collapse (numerical criterion):** adjacent rungs r, r′ collapse at context $L$ if $\max_{q \in Q}\, [H_r(q \mid L) - H_{r'}(q \mid L)] < \delta$ bits, over the COMPLETE statutory query set $\{$Q1, Q2, Q3, Q4 (statutory, by total; the rung difference equals the gap-part difference because the irreducible term is rung-invariant), Q5 per pair$\}$. **Frozen v0.2.5: $\delta = 0.01$ bits** (proposal §A; the corrected-kernel cell-max band around 0.01 is (0.003728, 0.046568), collapse horizons at $T_{ep}=14$ are $L^* = 2$ (r3→r4), 8 (r2→r3), 10 (r1→r2) at both ε).
- **Synchronization horizon** *(semantics amended v0.2.5, proposal §A′ and §B1)*: smallest $L < T_{ep}$ at which a rung's posterior entropy on a query falls below $\delta_{sync}$, evaluated on the **mean posterior entropy conditional on truncation**, $E[H \mid U > 0] = H_{law} / \Pr(U > 0)$, where $H_{law}$ is the law-mass mean and $\Pr(U > 0)$ the law's mass on windows with $U > 0$ (exact, since full-context windows are lossless at every rung; under uniform endpoints at multiples of $B$, $\Pr(U>0) = (T_{ep}-L)/T_{ep}$). $L = T_{ep}$ is the full-context control, not a point on the synchronization curve; a rung crossing $\delta_{sync}$ at no $L < T_{ep}$ is reported as not synchronized within the truncated domain. The law-mass mean (the learner's own law) and the per-endpoint curve at $T = T_{ep}$ are co-reported. For queries with an irreducible term under exact state knowledge (Q4 statutory, and any predictive target), the criterion applies to the observation-induced component $H_{total} - H_{irr}$; the irreducible term is reported alongside and is not subject to $\delta_{sync}$ (§A′ — under the total, Q4 could never synchronize below its irreducible term and the criterion would be vacuous for it). **Frozen v0.2.5: $\delta_{sync} = 0.01$ bits** (under it: $L^* = 12$ for r1, 10 for r2–r4, both ε).
- **Divergence thresholds $(\delta_p, \Delta_\tau)$** *(form v0.2.4, values v0.2.5)*: total-variation distances per §5's criterion form. **Frozen v0.2.5: $\Delta_\tau = 0.01$** (a broad stability region with reported sensitivity, not a flat band — see the proposal's §C as amended in its appended decision record: at $\delta_p = 10^{-2}$ the surface across the sixteen $L=2$ cells is exactly flat in 7, flat to ≤0.5% in 12, within 4.2% in all; within 27% at (14,4,2), corrected kernel). **$\delta_p$ is frozen as a reporting axis, not a scalar: $\{0, 10^{-4}, 10^{-3}, 10^{-2}, 3\cdot10^{-2}\}$**, with the exact $\delta_p = 0$ corner as anchor (pair_prob has no common stable plateau in $\delta_p$; the convention is kept visible per the proposal's §G2). Where a later experiment must name one $\delta_p$ for a class definition, that choice is recorded there, 0.01 the default.
...
*(v0.2.4 ledger:)* B1–B4 ($N_{supp}$, $M_{max}$, $\tau_{max}$, bit-length alarm; $H$ as recorded there) frozen at `d4-budget-freeze-v0.1.md`, commit `e921c74`. $m$, $W$, $\mathcal{T}$ frozen by this amendment (§5). $(\delta, \delta_{sync})$ and $(\delta_p, \Delta_\tau)$ **remain unfrozen** by design: three separate sweeps (§6), then a *second* stamped decision. Sequencing rationale: freezing thresholds before reading where the measured gaps fall would be premature collapse; freezing $m/W/\mathcal{T}$ now stops the forecast layer's parameters from drifting under it while the sweeps run.

*(v0.2.5 ledger:)* the second stamped decision is enacted — $\delta = 0.01$, $\delta_{sync} = 0.01$ (truncation-conditional, Q4 by gap part), $\Delta_\tau = 0.01$, $\delta_p$ as the five-point reporting axis with exact anchor — from `part2-threshold-freeze-proposal-v0.3.1.md` after the corrected-kernel sweep rerun (`sweep-rerun-comparison-2026-08-21.md`). The freeze commit is the second stamp; confirmatory threshold-dependent measurement additionally requires fresh held-out laws not used in the sweeps.

## §8 Dependency pinning

- Part I: `docs/yupana-m1-spec-draft.md` @ v0.2.4, revised in the same correction round as this v0.2.1 statute.
...
- CLAUDE.md founding snapshot at commit `0e50bc2`, with its authority re-graded by the status header added in this correction round.
...
Machine-checkable witness searches and controls. Exact-zero controls are executable assertions; positive predicted classes are executable searches whose empty result records a falsifier and triggers the stated redesign rule rather than being coerced into a passing test.

Symmetric witness quality is required (Part I v0.2.4): witnesses 1–3 must each be a **window-history class surviving beyond a single handcrafted transition** — established by enumeration under the declared window measure, reporting the fraction of windows where the field moves the posterior — so that no single rung (lineage included) becomes the protagonist by construction. *(Scoping, v0.2.2: witnesses 1–2 enumerate C1 windows — C0-family arity makes their classes provably empty (OBJECT determined by EVENT_KIND when $n_L, n_D \le 1$; owner-by-elimination when $n_T = 2$); witnesses 3 and 6 enumerate C0b windows as stated.)*

1. r2 > r1: a window class (C1) where OBJECT changes Q1's posterior.
2. r3 > r2: a window class (C1) where RELATED (owner on BLOCK) changes Q1/Q5's posterior.
3. r4 > r3: under truncated stochastic-order C0b windows, search for the predicted class where LINEAGE changes Q3's posterior; full context is an exact-zero control in both completion disciplines. Failure to find the class at the frozen validation horizon falsifies the proposed C0b mechanism or its scale — it is not automatically classified as a filter bug.
4. ctrl-irr: the decoy field changes **no** query ceiling (exact zero, not approximately).
5. ctrl-red: lineage changes no ceiling at full context in either FIFO or stochastic order under the enacted I5/ACTOR/lowest-free semantics.
6. Crossover: under truncated FIFO C0b windows, search for the predicted class where lineage changes a ceiling; failure has the same redesign-or-rescale interpretation as witness 3.
...
6. **Offset-unanchored truncation windows** (§2) — avoids handing over the clock explicitly, but length, RESET, and record content still leak position probabilistically; the joint-position filter must account for that evidence.
